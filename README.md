...
MipsStepLabは、MIPS命令を1ステップずつ実行しながら、レジスタ / HI/LO / メモリの変化を確認できる学習用ステップデバッガです。
...
現在は、Spring Bootを用いたWeb版を開発しています。  
...
CPU状態では、現在の レジスタ / HI/LO / メモリ の値を表示します。  
初期表示時点でも レジスタ / HI/LO / メモリ は0として表示します。
...
通常の利用用途では制限にかかりにくい設定にしていますが、ボタン連打などで短時間に大量のリクエストが発生した場合は、少し時間をおいてから再操作する必要があります。
